...
export function InteractiveGlobePreview() {
  ...
  return (
    <div className="w-full max-w-sm font-mono">
      <div className="border-4 border-amber-600 dark:border-amber-500 bg-amber-950 p-6 relative overflow-hidden">
        {/* Globe representation */}
        <div
          className="w-48 h-48 mx-auto relative border-4 border-amber-500 rounded-full bg-gradient-to-br from-blue-600 to-blue-900"
        ...
        >
          ...
          <div className="absolute inset-8 rounded-full border-2 border-amber-500/30" />
        ...
        <div className="flex justify-center gap-2 mt-4">
          <button
            ...
            className="px-4 py-2 border-4 border-amber-500 bg-amber-700 text-white font-bold uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
          >
            ← Rotate
          </button>
          <button
  ...
  return (
    <div className="w-full max-w-sm font-mono border-4 border-amber-600 dark:border-amber-500 bg-amber-50 dark:bg-amber-950">
      <div className="px-4 py-2 border-b-4 border-amber-500 bg-amber-200 dark:bg-amber-800 flex items-center gap-2">
        ...
        <span className="font-bold uppercase text-amber-900 dark:text-amber-100">■ Globe Markers</span>
      </div>
      <div className="divide-y-2 divide-amber-300 dark:divide-amber-700">
        {markers.map((marker, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 hover:bg-amber-100 dark:hover:bg-amber-900">
            <div className={`w-4 h-4 border-2 ${
              marker.status === "active" ? "border-green-600 bg-green-500" : "border-yellow-600 bg-yellow-500"
            } animate-pulse`} />
            <div className="flex-1">
              <div className="font-bold text-sm text-amber-900 dark:text-amber-100 uppercase">{marker.city}</div>
              <div className="text-xs text-amber-500">{marker.country}</div>
            </div>
            <div className="text-right text-xs text-amber-600 dark:text-amber-400 font-mono">
              <div>{marker.lat}</div>
              <div>{marker.lng}</div>
            </div>
          </div>
...
export function GlobeArcPreview() {
  ...
  return (
    <div className="w-full max-w-sm font-mono border-4 border-amber-600 dark:border-amber-500 bg-amber-50 dark:bg-amber-950">
      <div className="px-4 py-2 border-b-4 border-amber-500 bg-amber-200 dark:bg-amber-800 flex items-center gap-2">
        <Plane className="w-4 h-4 text-amber-700 dark:text-amber-300" />
        <span className="font-bold uppercase text-amber-900 dark:text-amber-100">■ Flight Routes</span>
      </div>
      <div className="p-4 space-y-3">
        {routes.map((route, i) => (
          <div key={i} className="p-3 border-4 border-amber-400 dark:border-amber-600 bg-amber-100 dark:bg-amber-900">
            <div className="flex items-center justify-between mb-2">
              <span className="font-bold text-amber-900 dark:text-amber-100">{route.from}</span>
              <div className="flex-1 mx-2 relative h-0.5 bg-amber-400">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 bg-amber-500 border-2 border-amber-700 rotate-45" />
              </div>
              <span className="font-bold text-amber-900 dark:text-amber-100">{route.to}</span>
            </div>
            <div className="flex justify-between text-xs text-amber-500 uppercase">
...
export function WorldMapPreview() {
  ...
  return (
    <div className="w-full max-w-md font-mono border-4 border-amber-600 dark:border-amber-500 bg-amber-50 dark:bg-amber-950">
      <div className="px-4 py-2 border-b-4 border-amber-500 bg-amber-200 dark:bg-amber-800 flex items-center gap-2">
        <Globe className="w-4 h-4 text-amber-700 dark:text-amber-300" />
        <span className="font-bold uppercase text-amber-900 dark:text-amber-100">■ World Map</span>
      </div>

      {/* Map representation */}
      <div className="p-4">
        <div className="aspect-video border-4 border-amber-500 bg-amber-950 relative overflow-hidden">
          ...
          {/* Labels */}
          <div className="absolute bottom-2 left-2 text-xs text-amber-500 uppercase">Click to select</div>
        </div>
      </div>

      {/* Legend */}
      <div className="grid grid-cols-2 gap-2 p-4 border-t-4 border-amber-400">
        {regions.map(region => (
          <button
            key={region.id}
            ...
            className={`p-2 border-4 text-left transition-all ${
              selectedRegion === region.id
                ? "border-amber-600 bg-amber-200 dark:bg-amber-800 shadow-[2px_2px_0px_0px_rgba(0,0,0,0.5)]"
                : "border-amber-400 dark:border-amber-600 hover:border-amber-500"
            }`}
          >
            <div className="text-xs font-bold uppercase text-amber-800 dark:text-amber-200">{region.name}</div>
            <div className="text-xs text-amber-500">{region.users} users</div>
          </button>
        ))}
      </div>
    </div>
